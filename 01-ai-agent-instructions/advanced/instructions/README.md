...
This folder contains the instruction set for the Spec Writer agent. The files are written for AI consumption - kept minimal and directive. This README is the human layer: it explains why each file exists and why it has its current shape.
...
Tool adapters (`.claude/`, `.github/`, `AGENTS.md`) use their own format and are not tagged. See the [manifest's adapter section](agents/spec-writer.manifest.md#files-that-are-adapters-only-do-not-load-as-instructions) for context.
...
**[Always load](agents/spec-writer.manifest.md#always-load)** — Every file in this section costs a file read on every session, regardless of what the user asks for. Keep it small.
